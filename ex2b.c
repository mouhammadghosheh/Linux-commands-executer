  ...
  while (1) {
    ...
    if ( in [0] == ' ' || in [0] == '\n') { //if a space or enter were entered it isn't counted as a word and if it was a space we tell the user
      ...
      if ( in [0] != '!') { //if the input was a normal command sent it to the file and then send it to the execvp
        ...
        int num = atoi(dest);
        char * str = Find_Command( in , fp, num); //this function finds the command from the file and sends it to the command function
        if (strcmp(str, " ") != 0) { //the function returns an empty string if number of lines entered exceeds total number of lines in file
          if (strcmp(str, "history\n") == 0) //if the command extracted was history send it to the history function
            ...
            commands(str); //send the command to execution
          }
        }
